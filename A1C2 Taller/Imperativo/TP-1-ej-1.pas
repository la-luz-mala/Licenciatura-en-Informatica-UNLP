...
Begin
  ...
  then begin
         ...
         if (posInferior <= posSuperior) then begin
            salto := posSuperior - posInferior + 1; {diferencia entre ambos puntos}
            for i:= posSuperior +1 to dimL do       {desde el primero que NO elimino hasta el final del vector}
                v[i-salto] := v[i];                 {reemplazo el primero q SI elimino por el primero que NO}
            dimL := dimL - salto;                   {reduzco la dimL}
        end;
